...
### 4. WhatsApp Evolution-API (QR login)

Evolution-API runs as a Baileys-based sidecar container that Chatwoot calls via
REST to provision WhatsApp inboxes through QR-code scanning (unofficial
WhatsApp Web bridge). No Twilio or Meta Business account required.

**Reconnect UX:** When a WhatsApp session disconnects (phone offline, session
expired), the agent opens the inbox settings in Chatwoot, a new QR code is
generated by Evolution-API, and the agent re-scans with the phone. No admin
intervention needed.
...
| File | Purpose |
|---|---|
| `docker-compose.fork.yaml` | Fork-specific compose file; adds `evolution-api` and `evolution-db-init` services and `evolution_instances` volume |
| `docker/install_evolution.md` | Admin setup guide |
| `.env.example` | Appended `EVOLUTION_API_URL` and `EVOLUTION_API_KEY` (fenced) |

**Fenced edits in existing files:**

| File | Fenced region |
|---|---|
| `docker-compose.fork.yaml` | `# FORK:BEGIN — Evolution-API WhatsApp provider service stack` wraps both new services |
| `docker-compose.fork.yaml` | `# FORK:BEGIN — Evolution sessions` wraps `evolution_instances` volume entry |
| `.env.example` | `# FORK:BEGIN — Evolution-API integration` wraps two new env var lines |

**Required `.env` vars (set before first `docker compose up`):**
...
`POSTGRES_PASSWORD` and `REDIS_PASSWORD` must also be set (already required
by the Rails stack). Evolution reuses the same Postgres and Redis containers
on separate database/index (`evolution` DB, Redis DB 1).
